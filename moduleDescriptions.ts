...
> = {
  ...
  [ModuleType.ModelAnalysisReport]: {
    ...
    category: "문서화",
    beginner:
      "내가 만든 분석 파이프라인을 '보고서 한 장'으로 깔끔하게 정리해 주는 마무리 블록입니다. 데이터가 무엇이고, 어떤 모델을 어떻게 학습·평가했는지, 결과가 어땠는지를 읽기 쉬운 HTML 문서로 만들어 줍니다. 파이프라인 맨 끝에 연결해 두고 실행하면, 위쪽 모듈들의 정보를 자동으로 모아 보고서를 완성합니다.",
    analysisMethod:
      "이 모듈은 데이터를 계산하는 분석 모듈이 아니라 '문서화(메타)' 모듈입니다. 연결된 곳에서 위로 거슬러 올라가며 LoadData/LoadClaimData(데이터·컬럼)·SplitData(분할 설정)·모델 정의(하이퍼파라미터)·TrainModel(특성/타깃)·EvaluateModel/EvaluateStat(지표·혼동행렬·임계값) 등 실행 결과 메타데이터를 모읍니다. 여기에 사용자가 넣은 추가 설명·PDF 텍스트를 합쳐, AI(generateAiText)가 자기완결 HTML 보고서를 작성합니다(API 키가 없으면 메타데이터만으로 동일 양식의 결정적 보고서를 만듭니다). 모든 수치는 실제 실행 결과에서 가져오며 지어내지 않습니다.",
    ...
    whenToUse: "모델/요율 개발을 마친 뒤 데이터·과정·결과를 문서로 남기고 싶을 때. 파이프라인 가장 마지막에 배치합니다.",
    connections: "EvaluateModel · EvaluateStat · SimulateAggDist · CombineLossModel 등 종단 모듈 → 모델 분석보고서. (출력 없음 = 말단)",
    commonErrors:
      "업스트림 모듈을 먼저 실행해 결과가 있어야 풍부한 보고서가 됩니다. API 키가 없으면 결정적 폴백 보고서가 생성됩니다(빈 결과 없음).",
    notes:
