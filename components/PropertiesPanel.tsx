...
import { CanvasModule, ModuleType, ModuleStatus, StatisticsOutput, Connection, DataPreview, TrainedModelOutput, StatsModelsResultOutput, FittedDistributionOutput, ExposureCurveOutput, XoLPriceOutput, FinalXolPriceOutput, EvaluationOutput, ColumnInfo, SplitDataOutput, MissingHandlerOutput, EncoderOutput, NormalizerOutput } from '../types';
import { PlayIcon, TableCellsIcon, CommandLineIcon, CogIcon, CodeBracketIcon, InformationCircleIcon, SparklesIcon, ClipboardIcon, CheckIcon } from './icons';
    ...
    const renderLine = (line: string) => {
        ...
        while ((result = combinedRegex.exec(line)) !== null) {
            ...
            if (result[2]) { // Bold
                ...
                parts.push(<code key={result.index} className="bg-gray-700 text-purple-300 px-1 py-0.5 rounded text-xs">{result[4]}</code>);
            }
        ...
        <div className="text-gray-300 space-y-2 text-sm">
            {text.split('\n').map((line, index) => {
                ...
                if (trimmedLine.startsWith('### ')) {
                    return <h4 key={index} className="text-md font-semibold mt-3 mb-1 text-gray-200">{renderLine(trimmedLine.substring(4))}</h4>
                }
                if (trimmedLine.startsWith('## ')) {
                     return <h3 key={index} className="text-lg font-semibold mt-4 mb-2 text-gray-100">{renderLine(trimmedLine.substring(3))}</h3>
                }
                 if (trimmedLine.startsWith('* ')) {
                    return <div key={index} className="flex items-start pl-2"><span className="mr-2 mt-1">•</span><div className="flex-1">{renderLine(trimmedLine.substring(2))}</div></div>
                }
                if (trimmedLine === '') {
                    return null;
                }
...
const AIModuleExplanation: React.FC<{ module: CanvasModule }> = ({ module }) => {
    ...
    const handleExplain = async () => {
        ...
        try {
            ...
            const defaultModuleData = DEFAULT_MODULES.find(m => m.type === module.type);
            const defaultParams = defaultModuleData ? defaultModuleData.parameters : {};

            const paramDetails = Object.keys(module.parameters).map(key => {
                const currentValue = module.parameters[key];
                const defaultValue = defaultParams[key];
                return `- **${key}**: (현재 값: \`${JSON.stringify(currentValue)}\`, 기본값: \`${JSON.stringify(defaultValue)}\`)`;
            }).join('\n');

            let optionsContext = '';
            if (module.type === ModuleType.NormalizeData && 'method' in module.parameters) {
                optionsContext = "\n\n**옵션 컨텍스트:**\n'method' 파라미터는 ['MinMax', 'StandardScaler', 'RobustScaler'] 옵션을 가집니다. 각 옵션의 차이점을 설명해 주세요.";
            } else if (module.type === ModuleType.SplitData && 'shuffle' in module.parameters) {
                optionsContext = "\n\n**옵션 컨텍스트:**\n'shuffle'과 'stratify' 파라미터는 ['True', 'False'] 옵션을 가집니다. 각 옵션이 언제 사용되는지 설명해 주세요.";
            } else if (module.type === ModuleType.StatModels && 'model' in module.parameters) {
    ...
    return (
        <div className="mt-4 border-t border-gray-700 pt-3">
            ...
            >
                ...
                {isLoading ? '생성 중...' : (show && explanation) ? '설명 숨기기' : 'AI로 파라미터 설명 보기'}
            </button>
            {show && (
                <div className="mt-2 p-3 bg-gray-700 rounded-lg">
                    {isLoading && <p className="text-sm text-gray-400">AI 설명을 생성하고 있습니다...</p>}
...
`;
            ...
            if (resultJson.feature_columns && resultJson.label_column) {
                const validFeatures = resultJson.feature_columns.filter((col: string) => inputColumns.includes(col));
                const validLabel = inputColumns.includes(resultJson.label_column) ? resultJson.label_column : null;
...
const PropertyGroup: React.FC<{ title: string; children: React.ReactNode, module: CanvasModule }> = ({ title, children, module }) => (
    <div className="mb-4">
...
const PropertyInput: React.FC<{ label: string; value: any; onChange: (value: any) => void; type?: string; step?: string }> = ({ label, value, onChange, type = "text", step }) => (
...
const PropertySelect: React.FC<{ label: string; value: any; onChange: (value: string) => void; options: (string | {label: string, value: string})[] }> = ({ label, value, onChange, options }) => (
    <div className="mb-3 last:mb-0">
        <label className="block text-sm text-gray-400 mb-1">{label}</label>
...
const PropertyDisplay: React.FC<{ label: string; value: React.ReactNode }> = ({ label, value }) => (
    <div className="mb-3 last:mb-0">
        <label className="block text-sm text-gray-400 mb-1">{label}</label>
...
const getConnectedDataSourceHelper = (moduleId: string, allModules: CanvasModule[], allConnections: Connection[], portNameToFind?: string): DataPreview | undefined => {
    const portName = portNameToFind || 'data_in';
    let inputConnection = allConnections.find(c => c.to.moduleId === moduleId && c.to.portName === portName);
    ...
    if (sourceModule.outputData.type === 'DataPreview') {
      ...
      return fromPortName === 'train_data_out' ? sourceModule.outputData.train : sourceModule.outputData.test;
    }
    return undefined;
};
...
const renderParameters = (
    ...
    const getConnectedDataSource = (moduleId: string, portNameToFind?: string) => 
        getConnectedDataSourceHelper(moduleId, allModules, allConnections, portNameToFind);
    ...
    switch (module.type) {
                ...
                if (folderHandle && (window as any).showOpenFilePicker) {
                    try {
                        ...
                        if (error.name !== 'AbortError') {
                            console.warn("Could not use directory picker, falling back to default.", error);
        ...
        case ModuleType.SelectData: {
            ...
            const availableDataTypes = ['string', 'number', 'boolean', 'category', 'datetime'];

            const currentSelections = module.parameters.columnSelections || {};

            const handleSelectionChange = (colName: string, key: 'selected' | 'type', value: boolean | string) => {
                const newSelections = {
                    ...currentSelections,
                    [colName]: {
                        ...(currentSelections[colName] || { selected: true, type: 'string' }),
                        [key]: value
                    }
                };
            ...
            if (inputColumns.length === 0) {
                return <p className="text-sm text-gray-500">Connect a data source module to configure columns.</p>;
            }
                ...
                <div className="flex flex-col">
                    ...
                    <div className="space-y-2 pr-2">
                        <div className="grid grid-cols-3 gap-2 items-center sticky top-0 bg-gray-800 py-1">
                            <span className="text-xs font-bold text-gray-400 col-span-2">Column Name</span>
                            <span className="text-xs font-bold text-gray-400">Data Type</span>
                        </div>
                        {inputColumns.map(col => {
                            const selection = currentSelections[col.name] || { selected: true, type: col.type };
                            return (
                                <div key={col.name} className="grid grid-cols-3 gap-2 items-center">
                                    <label className="flex items-center gap-2 text-sm truncate col-span-2" title={col.name}>
                                        <input
                                            type="checkbox"
                                            checked={selection.selected}
                                            onChange={e => handleSelectionChange(col.name, 'selected', e.target.checked)}
                                    ...
                                    <select 
                                        ...
                                        onChange={e => handleSelectionChange(col.name, 'type', e.target.value)}
                                    ...
                                    >
                                        {availableDataTypes.map(type => <option key={type} value={type}>{type}</option>)}
                                    </select>
                                </div>
                            )
                        })}
        ...
        case ModuleType.TransformData: {
            const handlerConnection = allConnections.find(c => c.to.moduleId === module.id && c.to.portName === 'handler_in');
            const handlerSourceModule = handlerConnection ? allModules.find(m => m.id === handlerConnection.from.moduleId) : undefined;
            const handler = handlerSourceModule?.outputData as (MissingHandlerOutput | EncoderOutput | NormalizerOutput | undefined);

            const sourceData = getConnectedDataSource(module.id, 'data_in');
            const allColumns = sourceData?.columns || [];
            const { primary_exclude_column = '', exclude_columns = [] } = module.parameters;

            if (!sourceData) return <p className="text-sm text-gray-500">Connect a data source to 'data_in'.</p>;
            if (!handler) return <p className="text-sm text-gray-500">Connect a handler (e.g., Prep Missing) to 'handler_in'.</p>;
            ...
            const handleSelectAll = (isSelect: boolean) => {
                if (isSelect) {
                    onParamChange('exclude_columns', [primary_exclude_column].filter(Boolean));
                } else {
                ...
                <div>
                    ...
                    <div className="mt-4">
                        <div className="flex justify-between items-center mb-2">
                            <h5 className="text-xs text-gray-500 uppercase font-bold">Columns to Transform</h5>
                            <div className="flex gap-2">
                                <button onClick={() => handleSelectAll(true)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md font-semibold">Select All</button>
                                <button onClick={() => handleSelectAll(false)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md font-semibold">Deselect All</button>
                            </div>
                        </div>
                        <div className="space-y-2 max-h-60 overflow-y-auto panel-scrollbar pr-2">
                            {allColumns.map(col => {
                                ...
                                } else if (handler.type === 'EncoderOutput' && col.type !== 'string') {
                                    isDisabled = true;
                                    disabledTooltip = 'Prep Encode only works on string columns.';
                                } else if (handler.type === 'NormalizerOutput' && col.type !== 'number') {
                                    isDisabled = true;
                                    disabledTooltip = 'Prep Normalize only works on number columns.';
                                }
                                return (
                                    <label key={col.name} className={`flex items-center gap-2 text-sm truncate ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`} title={disabledTooltip}>
                                        <input
                                            type="checkbox"
        ...
        case ModuleType.EncodeCategorical: {
            const sourceData = getConnectedDataSource(module.id);
            const categoricalColumns = (sourceData?.columns || []).filter(c => c.type === 'string');
            const { method, columns = [], handle_unknown, drop, ordinal_mapping } = module.parameters;
            ...
            if (!sourceData) {
                 return <p className="text-sm text-gray-500">Connect a data source to see available columns.</p>;
            }
            if (categoricalColumns.length === 0) {
                 return <p className="text-sm text-gray-500">No categorical (string) columns found in input data.</p>;
            }
            ...
            return (
                <>
                    ...
                    {method === 'ordinal' && (
                        <div className="mt-3 pt-3 border-t border-gray-700">
                            <label className="block text-sm text-gray-400 mb-1">Ordinal Mapping (JSON)</label>
                            <textarea
                                value={ordinal_mapping}
                                onChange={(e) => onParamChange('ordinal_mapping', e.target.value)}
                            ...
                            />
                             <p className="text-xs text-gray-500 mt-1">Define the order of categories for each column. Unmapped columns will be ordered alphabetically.</p>
                        </div>
                    )}

                     <div className="mt-4">
                        <h5 className="text-xs text-gray-500 uppercase font-bold mb-2">COLUMNS TO ENCODE</h5>
                        <p className="text-xs text-gray-500 mb-2">If none are selected, all string columns will be encoded.</p>
                        <div className="space-y-2 max-h-48 overflow-y-auto panel-scrollbar pr-2">
                            {categoricalColumns.map(col => (
                                <label key={col.name} className="flex items-center gap-2 text-sm truncate" title={col.name}>
                                    <input
                                        type="checkbox"
        ...
        case ModuleType.NormalizeData: {
            ...
            const handleSelectAll = (selectAll: boolean) => {
                const newSelections = { ...currentSelections };
                inputColumns.forEach(col => {
                    if (col.type === 'number') { // Only affect numeric columns
                        newSelections[col.name] = {
                            ...(currentSelections[col.name] || { type: col.type }),
                            selected: selectAll
                        };
            ...
            return (
                <>
                    <PropertySelect label="Method" value={module.parameters.method} onChange={v => onParamChange('method', v)} options={['MinMax', 'StandardScaler', 'RobustScaler']} />
                    
                    {inputColumns.length === 0 ? (
                        <p className="text-sm text-gray-500 mt-4">Connect a data source module to configure columns.</p>
                    ) : (
                        <div className="mt-4">
                            <h5 className="text-xs text-gray-500 uppercase font-bold mb-2">COLUMNS TO NORMALIZE</h5>
                            <div className="flex justify-end gap-2 mb-2">
                                <button onClick={() => handleSelectAll(true)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md font-semibold">Select All Numeric</button>
                                <button onClick={() => handleSelectAll(false)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md font-semibold">Deselect All</button>
                            </div>
                            <div className="space-y-2 pr-2">
                                <div className="grid grid-cols-5 gap-2 items-center sticky top-0 bg-gray-800 py-1">
                                    <span className="text-xs font-bold text-gray-400 col-span-3">Column Name</span>
                                    <span className="text-xs font-bold text-gray-400 col-span-2">Data Type</span>
                                </div>
                                {inputColumns.map(col => {
                                    const selection = currentSelections[col.name] || { selected: false, type: col.type };
                                    return (
                                        <div key={col.name} className="grid grid-cols-5 gap-2 items-center">
                                            <label className="flex items-center gap-2 text-sm truncate col-span-3" title={col.name}>
                                                <input
                                                    type="checkbox"
                                                    checked={selection.selected}
                                                    onChange={e => handleSelectionChange(col.name, e.target.checked)}
                                                ...
                                                />
                                                <span className={`truncate ${col.type !== 'number' ? 'text-gray-500' : ''}`}>{col.name}</span>
                                            </label>
                                            <div className="col-span-2">
                                                <span className="text-xs bg-gray-700 text-gray-300 px-2 py-1 rounded-md">{col.type}</span>
                                            </div>
                                        </div>
                                    )
                                })}
        ...
        case ModuleType.TransitionData: {
            const sourceData = getConnectedDataSource(module.id);
            const numericColumns = (sourceData?.columns || []).filter(c => c.type === 'number');
            ...
            return (
                <div>
                     <div className="flex justify-between items-center mb-2">
                        <h5 className="text-xs text-gray-500 uppercase font-bold">Column Transformations</h5>
                        <div title={formulaTooltip}>
                            <InformationCircleIcon className="w-5 h-5 text-gray-400 cursor-help" />
                        </div>
                    </div>
                    {numericColumns.length === 0 ? (
                         <p className="text-sm text-gray-500">No numeric columns found in the input data.</p>
                    ) : (
                        <div className="space-y-3">
                        {numericColumns.map(col => (
                            <div key={col.name} className="grid grid-cols-2 gap-2 items-center">
                                <label className="text-sm truncate" title={col.name}>{col.name}</label>
                                <select 
                                    value={transformations[col.name] || 'None'}
                                    onChange={e => handleTransformChange(col.name, e.target.value)}
                                    className="w-full bg-gray-700 border border-gray-600 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500"
                                >
        ...
        case ModuleType.ResampleData: {
            ...
            if (inputColumns.length === 0) {
                return <p className="text-sm text-gray-500">Connect a data source to configure resampling.</p>;
            }
            ...
            return (
                <>
                    ...
                    <PropertySelect 
                        ...
                        onChange={v => onParamChange('target_column', v === '' ? null : v)}
        ...
        case ModuleType.SplitData: {
            ...
            if (inputColumns.length === 0 && !sourceData) {
                return <p className="text-sm text-gray-500">Connect a data source module to configure parameters.</p>;
            }
        
            return <>
                <PropertyInput label="Train Size" type="number" value={module.parameters.train_size} onChange={v => onParamChange('train_size', v)} />
                <PropertyInput label="Random State" type="number" value={module.parameters.random_state} onChange={v => onParamChange('random_state', v)} />
                <PropertySelect label="Shuffle" value={module.parameters.shuffle} onChange={v => onParamChange('shuffle', v)} options={['True', 'False']} />
                <PropertySelect label="Stratify" value={module.parameters.stratify} onChange={v => onParamChange('stratify', v)} options={['False', 'True']} />
        ...
        case ModuleType.LinearRegression:
            return <>
                ...
                <PropertySelect 
                    ...
                    options={['True', 'False']} 
                />
                {(['Lasso', 'Ridge', 'ElasticNet'].includes(module.parameters.model_type)) && (
                     <PropertyInput 
        ...
        case ModuleType.ResultModel: {
            ...
            if (inputColumns.length === 0) {
                return <p className="text-sm text-gray-500">Connect a data source to the 'data_in' port to configure.</p>;
            }
            ...
            const handleLabelChange = (colName: string) => {
                ...
                if (newLabel && feature_columns.includes(newLabel)) {
                    onParamChange('feature_columns', feature_columns.filter((c: string) => c !== newLabel));
                }
            };
            ...
            return (
                <div>
                    ...
                    />
                    <div className="mb-4">
                        <h5 className="text-xs text-gray-500 uppercase font-bold mb-2">Label Column</h5>
                         <select
                            value={label_column || ''}
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <h5 className="text-xs text-gray-500 uppercase font-bold">Feature Columns</h5>
                            <div className="flex gap-2">
                                <button 
                            ...
                            {inputColumns.map(col => (
                                <label key={col.name} className="flex items-center gap-2 text-sm truncate" title={col.name}>
                                    <input
                                        ...
                                        onChange={e => handleFeatureChange(col.name, e.target.checked)}
        ...
        case ModuleType.PrincipalComponentAnalysis: {
            const sourceData = getConnectedDataSource(module.id);
            const inputColumns = (sourceData?.columns || []).filter(c => c.type === 'number').map(c => c.name);

            if (inputColumns.length === 0) {
                return <p className="text-sm text-gray-500">Connect a data source with numeric columns to configure.</p>;
            }
            ...
            return (
                <>
                    {Object.entries(otherParams).map(([key, value]) => (
                        <PropertyInput key={key} label={key} value={value} type="number" onChange={v => onParamChange(key, v)} />
                    ))}
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <h5 className="text-xs text-gray-500 uppercase font-bold">Feature Columns</h5>
                            <div className="flex gap-2">
                                <button onClick={() => handleSelectAllFeatures(true)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md font-semibold">Select All</button>
                                <button onClick={() => handleSelectAllFeatures(false)} className="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md font-semibold">Deselect All</button>
                            </div>
                        </div>
                        <p className="text-xs text-gray-500 mb-2">If none are selected, all numeric columns will be used.</p>
                        <div className="space-y-2 pr-2 max-h-40 overflow-y-auto panel-scrollbar">
                            {inputColumns.map(col => (
                                <label key={col} className="flex items-center gap-2 text-sm truncate" title={col}>
                                    <input
                                        type="checkbox"
        ...
        case ModuleType.LogisticRegression:
            return <>
                ...
                <PropertySelect label="Penalty" value={module.parameters.penalty} onChange={v => onParamChange('penalty', v)} options={['l1', 'l2', 'elasticnet', 'none']} />
                <PropertyInput label="C (Regularization)" type="number" value={module.parameters.C} onChange={v => onParamChange('C', v)} step="0.1" />
                <PropertySelect label="Solver" value={module.parameters.solver} onChange={v => onParamChange('solver', v)} options={['lbfgs', 'newton-cg', 'liblinear', 'sag', 'saga']} />
                <PropertyInput label="Max Iterations" type="number" value={module.parameters.max_iter} onChange={v => onParamChange('max_iter', v)} />
            </>;
         case ModuleType.NaiveBayes:
            return <>
                <PropertyDisplay label="Model Purpose" value="Classification" />
                <PropertyInput label="Var Smoothing" type="number" value={module.parameters.var_smoothing} onChange={v => onParamChange('var_smoothing', v)} step="1e-10" />
            </>;
        case ModuleType.DecisionTree: {
            ...
            const handlePurposeChange = (newPurpose: string) => {
                ...
                if (newPurpose === 'classification' && !['gini', 'entropy'].includes(module.parameters.criterion)) {
                    onParamChange('criterion', 'gini');
                } else if (newPurpose === 'regression' && !['squared_error', 'friedman_mse', 'absolute_error', 'poisson'].includes(module.parameters.criterion)) {
                    onParamChange('criterion', 'squared_error');
                }
            };
            return <>
                <PropertySelect label="Model Purpose" value={purpose} onChange={handlePurposeChange} options={['classification', 'regression']} />
                <PropertyInput label="Max Depth" type="number" value={module.parameters.max_depth} onChange={v => onParamChange('max_depth', v)} />
                <PropertySelect 
                    label="Criterion" 
                    value={module.parameters.criterion} 
                    onChange={v => onParamChange('criterion', v)} 
                    options={purpose === 'classification' ? ['gini', 'entropy'] : ['squared_error', 'friedman_mse', 'absolute_error', 'poisson']} 
                />
            </>;
        }
        case ModuleType.RandomForest: {
            ...
            const handlePurposeChange = (newPurpose: string) => {
                ...
                if (newPurpose === 'classification' && !['gini', 'entropy'].includes(module.parameters.criterion)) {
                    onParamChange('criterion', 'gini');
                } else if (newPurpose === 'regression' && !['squared_error', 'absolute_error', 'poisson'].includes(module.parameters.criterion)) {
                    onParamChange('criterion', 'squared_error');
                }
            };
            return <>
                <PropertySelect label="Model Purpose" value={purpose} onChange={handlePurposeChange} options={['classification', 'regression']} />
                <PropertyInput label="N Estimators" type="number" value={module.parameters.n_estimators} onChange={v => onParamChange('n_estimators', v)} />
                <PropertyInput label="Max Depth" type="number" value={module.parameters.max_depth} onChange={v => onParamChange('max_depth', v)} />
                <PropertySelect 
                    label="Criterion" 
                    value={module.parameters.criterion} 
                    onChange={v => onParamChange('criterion', v)} 
                    options={purpose === 'classification' ? ['gini', 'entropy'] : ['squared_error', 'absolute_error', 'poisson']} 
                />
            </>;
        }
        case ModuleType.SVM:
            return <>
                <PropertySelect label="Model Purpose" value={module.parameters.model_purpose} onChange={v => onParamChange('model_purpose', v)} options={['classification', 'regression']} />
                <PropertyInput label="C (Regularization)" type="number" value={module.parameters.C} onChange={v => onParamChange('C', v)} step="0.1" />
                <PropertySelect label="Kernel" value={module.parameters.kernel} onChange={v => onParamChange('kernel', v)} options={['rbf', 'linear', 'poly', 'sigmoid']} />
                <PropertyInput label="Gamma" value={module.parameters.gamma} onChange={v => onParamChange('gamma', v)} />
            </>;
        ...
        case ModuleType.StatModels:
            return (
                <PropertySelect 
                    ...
                    options={['OLS', 'Logit', 'Poisson', 'NegativeBinomial', 'Gamma', 'Tweedie']} 
                />
            );
        default:
             const hasParams = Object.keys(module.parameters).length > 0;
            if (!hasParams) {
                 return <p className="text-sm text-gray-500">This module has no configurable parameters.</p>;
            }
             return (
                <div>
                    {Object.entries(module.parameters).map(([key, value]) => {
                        if (typeof value === 'boolean') {
                            return <PropertySelect key={key} label={key} value={value ? 'True' : 'False'} onChange={v => onParamChange(key, v === 'True')} options={['True', 'False']} />
                        }
                        if (typeof value === 'number') {
                            return <PropertyInput key={key} label={key} value={value} type="number" onChange={v => onParamChange(key, v)} />
                        }
                         return <PropertyInput key={key} label={key} value={value} onChange={v => onParamChange(key, v)} />
                    })}
                </div>
             )
    }
...
const StatRow: React.FC<{ label: string; value: React.ReactNode }> = ({ label, value }) => (
    <div className="flex justify-between items-center text-sm py-1.5 px-2 border-b border-gray-700 last:border-b-0">
        ...
        <span className="font-mono text-gray-200 font-medium truncate" title={String(value)}>{value}</span>
    </div>
);
...
const ColumnInfoTable: React.FC<{
    ...
    <div className="text-sm">
        {columns.map(col => {
            ...
            const colorClass = highlight.color ? `text-${highlight.color}-400` : 'text-gray-200';
            const strikethroughClass = highlight.strikethrough ? 'line-through' : '';

            return (
                <div key={col.name} className="flex justify-between items-center py-1 px-2 border-b border-gray-700 last:border-b-0">
                    <span className={`font-mono truncate ${colorClass} ${strikethroughClass}`}>{col.name}</span>
...
const DataStatsSummary: React.FC<{ data: DataPreview; title?: string }> = ({ data, title }) => {
    const numericCols = useMemo(() => data.columns.filter(c => c.type === 'number'), [data.columns]);
    const rows = useMemo(() => data.rows || [], [data.rows]);

    return (
        <div>
            {title && <h4 className="text-xs text-gray-500 uppercase font-bold mb-2">{title}</h4>}
            <div className="space-y-2">
                {numericCols.map(col => {
                    const values = rows.map(r => r[col.name] as number).filter(v => typeof v === 'number' && !isNaN(v));
...
const MissingValueSummary: React.FC<{ data: DataPreview; title?: string }> = ({ data, title }) => {
    const missingCounts = useMemo(() => {
        const counts: Record<string, number> = {};
        if (!data.columns) return [];
        data.columns.forEach(col => {
            counts[col.name] = (data.rows || []).filter(row => row[col.name] == null || row[col.name] === '').length;
        });
    ...
    return (
        <div>
            ...
            <h5 className="text-xs text-gray-500 uppercase font-bold my-2">Missing Values per Column</h5>
            {missingCounts.length > 0 ? (
                <div className="max-h-96 overflow-y-auto panel-scrollbar pr-2">
                    {missingCounts.map(([name, count]) => {
                        const percentage = data.totalRowCount > 0 ? ((count / data.totalRowCount) * 100).toFixed(1) : '0.0';
                        return <StatRow key={name} label={name} value={`${count} (${percentage}%)`} />;
                    })}
                </div>
            ) : (
                <p className="text-sm text-gray-500 text-center p-2">No columns in input data.</p>
            )}
        </div>
    );
};

const DataTableStats: React.FC<{ data: DataPreview; title?: string; highlightedColumns?: string[] }> = ({ data, title, highlightedColumns = [] }) => {
    const numericCols = useMemo(() => data.columns.filter(c => c.type === 'number'), [data.columns]);
    const rows = useMemo(() => data.rows || [], [data.rows]);
    ...
    return (
        <div>
            {title && <h3 className="text-md font-semibold mb-2 text-gray-300">{title}</h3>}
                ...
                <div className="text-gray-200">
                    ...
                    <div className="max-h-96 overflow-y-auto panel-scrollbar">
                        {Object.keys(colStats).map((colName) => {
                            ...
                            return (
                                <div key={colName} className="grid grid-cols-5 gap-4 px-4 py-2.5 text-sm border-b border-gray-800 last:border-b-0">
                                    <div className={`font-mono truncate ${isHighlighted ? 'text-red-400' : ''}`} title={colName}>{colName}</div>
                                    <div className="font-mono text-right">{stats.mean.toFixed(2)}</div>
                                    <div className="font-mono text-right">{stats.std.toFixed(2)}</div>
                                    <div className="font-mono text-right">{stats.min.toFixed(2)}</div>
                                    <div className="font-mono text-right">{stats.max.toFixed(2)}</div>
                                </div>
                            );
                        })}
                         {Object.keys(colStats).length === 0 && <p className="text-sm text-gray-500 text-center p-4">No numeric columns to display stats for.</p>}
                    </div>
                </div>
            </div>
        </div>
...
const PanelModelMetrics: React.FC<{ metrics: Record<string, string | number> }> = ({ metrics }) => (
    <div>
        ...
        <div className="bg-gray-800 rounded-lg p-3 space-y-2">
            {Object.entries(metrics).map(([key, value]) => (
                <StatRow key={key} label={key} value={typeof value === 'number' ? Number(value).toFixed(4) : value} />
            ))}
        </div>
    </div>
);
...
export const PropertiesPanel: React.FC<PropertiesPanelProps> = ({
  ...
  const [activePreviewTab, setActivePreviewTab] = useState<'Input' | 'Output'>('Input');
...
const getConnectedModelSource = useCallback((moduleId: string): CanvasModule | undefined => {
...
const getConnectedDataSource = useCallback((moduleId: string, portNameToFind?: string) => {
    return getConnectedDataSourceHelper(moduleId, modules, connections, portNameToFind);
}, [modules, connections]);

const renderInputPreview = () => {
    if (!module) return null;

    const handlerConnection = connections.find(c => c.to.moduleId === module.id && c.to.portName === 'handler_in');
    const handlerSourceModule = handlerConnection ? modules.find(m => m.id === handlerConnection.from.moduleId) : undefined;
    const handler = handlerSourceModule?.outputData as (MissingHandlerOutput | EncoderOutput | NormalizerOutput | undefined);

    if (module.type === ModuleType.TransformData) {
        if (!handler) {
            return <div className="text-center text-gray-500 p-4">Connect a handler module to 'handler_in'.</div>;
        }
        return (
            <div>
                <h4 className="text-xs text-gray-500 uppercase font-bold mb-2">Input Handler</h4>
                <StatRow label="Type" value={handlerSourceModule?.name} />
                <StatRow label="Method" value={handler.method} />
                {(handler as MissingHandlerOutput).strategy && <StatRow label="Strategy" value={(handler as MissingHandlerOutput).strategy} />}
            </div>
        );
    }
    
    if (module.type === ModuleType.LoadData || module.type === ModuleType.XolLoading) {
        return <StatRow label="File Name" value={module.parameters.source || 'N/A'} />;
    }
    ...
    if (!inputData) {
        return <div className="text-center text-gray-500 p-4">Input data not available. Connect a preceding module.</div>;
    }
    
    switch (module.type) {
        case ModuleType.ResampleData: {
            const targetColumn = module.parameters.target_column;
            if (!targetColumn) return <p className="text-sm text-gray-500">Select a target column to see value counts.</p>;
            ...
            return (
                <div>
                    <h4 className="text-xs text-gray-500 uppercase font-bold mb-2">Value Counts for '{targetColumn}'</h4>
        ...
        case ModuleType.EncodeCategorical: {
            const categoricalColumns = inputData.columns.filter(c => c.type === 'string');
            return (
                <div>
                    <h4 className="text-xs text-gray-500 uppercase font-bold mb-2">Categorical Columns Found</h4>
                    {categoricalColumns.length > 0 ? (
                        <ColumnInfoTable columns={categoricalColumns} />
                    ) : (
                        <p className="text-sm text-gray-500">No string columns to encode.</p>
                    )}
                </div>
            );
        }
        ...
        case ModuleType.ScoreModel:
            return (
                <div>
                    <StatRow label="Total Rows" value={inputData.totalRowCount.toLocaleString()} />
...
const renderOutputPreview = () => {
    if (!module || module.status !== ModuleStatus.Success || !module.outputData) {
        return <div className="text-center text-gray-500 p-4">Run the module successfully to see the output.</div>;
    }
    const outputData = module.outputData;

    const visualizableTypes = [
        'DataPreview', 'StatisticsOutput', 'SplitDataOutput', 'TrainedModelOutput', 
        'StatsModelsResultOutput', 'XoLPriceOutput', 'FinalXolPriceOutput', 
        'EvaluationOutput', 'KMeansOutput', 'HierarchicalClusteringOutput', 
        'PCAOutput', 'DBSCANOutput'
    ];
    
    const canVisualize = () => {
        if (!module || !module.outputData) return false;
        if (visualizableTypes.includes(module.outputData.type)) return true;
        if (['KMeansOutput', 'HierarchicalClusteringOutput', 'DBSCANOutput'].includes(module.outputData.type)) return true;
        return false;
    };

    const renderTitle = (title: string) => <h3 className="text-md font-semibold mb-2 text-gray-300">{title}</h3>;
    
    const previewContent = (() => {
        switch (module.type) {
            case ModuleType.LoadData:
                if (outputData.type === 'DataPreview') {
                    return <><h3 className="text-md font-semibold mb-2 text-gray-300">Column Structure</h3><ColumnInfoTable columns={outputData.columns} /></>;
                }
                break;
            case ModuleType.Statistics:
                                ...
                                <div className="text-gray-200">
                                    ...
                                    <div className="max-h-96 overflow-y-auto panel-scrollbar">
                                        {Object.keys(outputData.stats).map((col) => {
                                            ...
                                            return (
                                                <div key={col} className="grid grid-cols-4 gap-4 px-4 py-2.5 text-sm border-b border-gray-800 last:border-b-0">
                                                    <div className="font-mono truncate" title={col}>{col}</div>
                                                    <div className="font-mono text-right">{typeof columnStats.mean === 'number' && !isNaN(columnStats.mean) ? columnStats.mean.toFixed(2) : 'N/A'}</div>
                                                    <div className="font-mono text-right">{typeof columnStats['50%'] === 'number' && !isNaN(columnStats['50%']) ? columnStats['50%'].toFixed(2) : 'N/A'}</div>
                                                    <div className="font-mono text-right">{columnStats.nulls}</div>
                                                </div>
                                            );
                                        })}
                                    </div>
            ...
            case ModuleType.SelectData:
                if (outputData.type === 'DataPreview') {
                    ...
                    const removedCols = originalCols.filter(c => !newCols.includes(c));
                    ...
                    return <ColumnInfoTable columns={originalData?.columns || []} highlights={highlights} />;
                }
                break;
            case ModuleType.ResampleData: {
                const inputData = getConnectedDataSource(module.id);
                ...
                if (outputData.type === 'SplitDataOutput') {
                    return (
                        <div className="space-y-4">
                            <DataStatsSummary data={outputData.train} title="Train Data Summary" />
                            <DataStatsSummary data={outputData.test} title="Test Data Summary" />
                        </div>
                    );
                }
                break;
            case ModuleType.TrainModel:
                if (outputData.type === 'TrainedModelOutput') {
                    const { modelType, coefficients, intercept, featureColumns, labelColumn } = outputData;

                    const complexModels = [
                        ModuleType.DecisionTree, ModuleType.RandomForest, ModuleType.SVM,
                        ModuleType.KNN, ModuleType.NaiveBayes, ModuleType.LinearDiscriminantAnalysis
                    ];
                    ...
                    return (
                        <div className="space-y-4">
                            {formulaParts.length > 0 && (
                                 <div>
                                    <h4 className="text-xs text-gray-500 uppercase font-bold mb-2">Model Equation</h4>
            ...
            default:
                if (outputData.type === 'DataPreview') {
                    return <MissingValueSummary data={outputData} title="Output Data Summary" />;
                }
                break;
        }
        return <div className="text-center text-gray-500 p-4">No specific preview for this module's output.</div>;
    })();
  ...
  return (
    <div ref={resizableContainerRef} className="bg-gray-800 text-white h-full flex flex-col">
      ...
      <div className="flex-grow flex flex-col min-h-0">
        <div className="p-3 border-b border-gray-700 flex-shrink-0">
          <input
            ...
            disabled={!module}
          />
          <p className="text-xs text-gray-500 mt-1">{module ? module.type : 'No module selected'}</p>
        </div>
          ...
          <div className="flex-shrink-0 border-b border-gray-700">
            <div className="flex">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 p-3 text-sm font-semibold ${activeTab === 'properties' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700/50'}`}
              >
                <CogIcon className="w-5 h-5" /> Properties
              </button>
              <button
                onClick={() => setActiveTab('preview')}
                className={`flex-1 flex items-center justify-center gap-2 p-3 text-sm font-semibold ${activeTab === 'preview' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700/50'}`}
              >
                <TableCellsIcon className="w-5 h-5" /> Preview
              </button>
              <button
                onClick={() => setActiveTab('code')}
                className={`flex-1 flex items-center justify-center gap-2 p-3 text-sm font-semibold ${activeTab === 'code' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700/50'}`}
              >
        ...
        <div className="flex-grow overflow-y-auto panel-scrollbar p-3">
          {!module ? (
            <div className="flex items-center justify-center h-full">
              <p className="text-gray-500">Select a module to see its properties.</p>
            </div>
          ) : (
            <>
              {activeTab === 'properties' && (
                <PropertyGroup title="Parameters" module={module}>
                  {renderParameters(module, handleParamChange, fileInputRef, modules, connections, projectName, updateModuleParameters, handleLoadSample, folderHandle)}
                </PropertyGroup>
              )}
              {activeTab === 'preview' && (
                <div>
                    <div className="flex mb-3 rounded-md bg-gray-700 p-1">
                        <button onClick={() => setActivePreviewTab('Input')} className={`flex-1 text-center text-sm py-1 rounded-md transition-colors ${activePreviewTab === 'Input' ? 'bg-gray-600 font-semibold' : 'hover:bg-gray-600/50'}`}>Input</button>
                        <button onClick={() => setActivePreviewTab('Output')} className={`flex-1 text-center text-sm py-1 rounded-md transition-colors ${activePreviewTab === 'Output' ? 'bg-gray-600 font-semibold' : 'hover:bg-gray-600/50'}`}>Output</button>
                    </div>
                    <div className="bg-gray-900/50 p-3 rounded-lg">
                        {activePreviewTab === 'Input' ? renderInputPreview() : renderOutputPreview()}
                    </div>
                </div>
              )}
              {activeTab === 'code' && (
      ...
      <div className="flex-shrink-0 flex flex-col" style={{ height: `${terminalHeight}px` }}>
        <div 
        ...
        >
            {logs.map(log => (
                <div key={log.id} className="flex group hover:bg-gray-800/50 rounded px-1 py-0.5">
                    <span className="text-gray-500 mr-2 flex-shrink-0 select-none">{log.timestamp}</span>
                    <span className={`mr-2 font-bold flex-shrink-0 select-none ${
