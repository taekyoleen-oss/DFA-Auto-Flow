...
// 없으면 포트 타입 기반 기본 문구로 폴백. DFA 도메인 포트 타입
// (contract/threshold/distribution/curve)까지 포함.
const genericPortHint = (portType: string, isInput: boolean): string => {
  const map: Record<string, [string, string]> = {
    // [입력 문구, 출력 문구]
    data: ['데이터를 연결하세요', '데이터가 나옵니다'],
    model: ['모델을 연결하세요', '모델이 나옵니다'],
    evaluation: ['평가/결과를 연결하세요', '평가·결과가 나옵니다'],
    contract: ['XoL 계약(정의)을 연결하세요', 'XoL 계약이 나옵니다'],
    threshold: ['임계값(threshold)을 연결하세요', '임계값이 나옵니다'],
    distribution: ['적합된 분포를 연결하세요', '적합된 분포가 나옵니다'],
    curve: ['익스포저 곡선을 연결하세요', '익스포저 곡선이 나옵니다'],
  };
