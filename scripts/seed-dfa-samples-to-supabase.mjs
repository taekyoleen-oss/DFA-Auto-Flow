/**
 * DFA-Auto-Flow samples-list.json을 Supabase에 등록하는 스크립트
 *
 * 사용법: node scripts/seed-dfa-samples-to-supabase.mjs
 *
 * .env에 VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY (또는 NEXT_PUBLIC_*) 가 있어야 합니다.
 * public/samples-list.json을 읽어 app_section "DFA"로 등록합니다.
 */
...
const samplesPath = resolve(projectRoot, "public", "samples-list.json");
if (!existsSync(samplesPath)) {
  console.error("public/samples-list.json을 찾을 수 없습니다.");
  process.exit(1);
}
...
try {
  ...
  console.error("samples-list.json 파싱 실패:", e.message);
  process.exit(1);
}

if (!Array.isArray(items)) {
  console.error("samples-list.json은 배열이어야 합니다.");
  process.exit(1);
}

async function main() {
  console.log(`DFA 샘플 ${items.length}건 등록 시작 (app_section: DFA)`);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (loadDataModule?.parameters?.fileContent) {
      ...
      const { data: inputRow, error: inputErr } = await supabase
        ...
        .select("id")
        .single();
      if (!inputErr && inputRow) inputDataId = inputRow.id;
    }
  ...
  console.log("\n완료. DFA-Auto-Flow Samples 메뉴에서 확인하세요.");
}
