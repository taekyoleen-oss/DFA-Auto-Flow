...
//   파라미터·출력 메타데이터를 수집한다(안전 가드: 없으면 생략). DFA의 계리 도메인 모듈
//   (XoL/Diversion/claims 등)과 일부 ML 모듈을 모두 다루는 **범용 수집**이다.
...
// 모델 정의(지도/비지도/계리) 모듈 유형 → 한글 라벨.
// DFA enum에 존재하는 타입만 키로 사용한다(없는 ML 타입 참조 시 컴파일 오류 방지).
const MODEL_KIND_LABEL: Partial<Record<ModuleType, string>> = {
  ...
  [ModuleType.PrincipalComponentAnalysis]: "PCA(주성분 분석)",
  [ModuleType.OLSModel]: "OLS(최소제곱 회귀)",
  [ModuleType.LogisticModel]: "Logistic GLM",
  [ModuleType.PoissonModel]: "Poisson GLM",
  [ModuleType.QuasiPoissonModel]: "Quasi-Poisson GLM",
  [ModuleType.NegativeBinomialModel]: "Negative Binomial GLM",
};
...
// DataPreview 메타를 ctx에 적용(DFA 래퍼 출력의 .data도 동일 처리).
function applyDataPreview(ctx: ReportContext, dp: any): void {
  if (!dp) return;
  if (ctx.rowCount === undefined) {
    if (typeof dp.totalRowCount === "number") ctx.rowCount = dp.totalRowCount;
    else if (Array.isArray(dp.rows)) ctx.rowCount = dp.rows.length;
  }
  if (ctx.columnCount === undefined && Array.isArray(dp.columns)) {
    ctx.columnCount = dp.columns.length;
  }
  if (!ctx.columns && Array.isArray(dp.columns)) {
    const sampleRow = (dp.rows && dp.rows[0]) || {};
    ctx.columns = dp.columns.map((c: any) => ({
      name: c.name,
      type: c.type,
      sample:
  ...
  if (!ctx.sampleRows && Array.isArray(dp.rows)) {
    ctx.sampleRows = dp.rows.slice(0, 5);
  }
}

// 어떤 outputData에서든 내부 DataPreview를 꺼내본다(DFA 래퍼 대응).
function extractDataPreview(od: any): any | null {
  if (!od) return null;
  if (od.type === "DataPreview") return od;
  // ClaimDataOutput / InflatedDataOutput / FormatChangeOutput 등은 .data에 DataPreview를 담는다.
  if (od.data && od.data.type === "DataPreview") return od.data;
  return null;
}
 ...
 * DFA 도메인(계리) + 일부 ML 모듈을 모두 다루는 범용 수집(안전 가드 포함).
 */
export function gatherReportContext(
  ...
  // 1) report_in*으로 들어오는 연결에서 시작해 모든 조상(ancestors) BFS 수집.
  ...
  // 2) LoadData / LoadClaimData — 데이터셋 메타.
  const loadData =
    find(ModuleType.LoadData) ||
    find(ModuleType.LoadClaimData) ||
  ...
  if (loadData) {
    ...
    applyDataPreview(ctx, extractDataPreview(loadData.outputData));
  }

  // 2-b) 행/열 수 보강: LoadData 출력이 없거나 미설정이면 다른 조상의
  // DataPreview(직접/래퍼/Split train+test)에서 유추한다.
  if (ctx.rowCount === undefined || ctx.columnCount === undefined || !ctx.columns) {
    for (const m of ancestors) {
      const od: any = m.outputData;
      if (!od) continue;
      const dp = extractDataPreview(od);
      if (dp) applyDataPreview(ctx, dp);
  ...
  // 6-b) 통계 모델 평가(EvaluateStat) — 있으면 메트릭을 범용 수집.
  const evalStat = ancestors.find((m) => (m.outputData as any)?.type === "EvaluateStatOutput");
  if (evalStat && !ctx.metrics) {
    const od: any = evalStat.outputData;
    if (od && od.metrics) {
      ctx.metrics = od.metrics;
      ctx.modelType = ctx.modelType || String(od.modelType || "통계 모델");
    }
  }
 ...
 * 토큰 한도를 초과하므로 반드시 제거한다(폴백 pipelineSteps와 동일 제외).
...
<article class="report">
  <header class="cover">
    ...
    <p class="meta">생성: ${escapeHtml(generatedAt)} · 작성: DFA Auto Flow · 모델 유형: ${escapeHtml(
    ctx.modelType || "(자료 없음)"
  )}</p>
  </header>
  ...
  <ul>
    ...
    <li>더 풍부한 해설(도메인 배경·개선 제안)은 상단 설정에서 AI API 키를 입력하면 AI 생성 보고서로 보강됩니다.</li>
  </ul>
  ${extra}

  <footer>본 보고서는 DFA Auto Flow의 실제 파이프라인 메타데이터를 기반으로 자동 생성되었습니다. · 모듈: <b>모델 분석보고서</b> · 생성 도구: DFA Auto Flow (결정적 폴백)</footer>
</article>
</body>
</html>`;
}
