 ...
 *
 * 샘플/저장 모델의 LoadData/LoadClaimData 모듈은 데이터 본문 대신 파일명(source)만
 * 가질 수 있다. 이 모듈은 파일명을 실제 CSV 본문으로 "해석(resolve)"한다. 우선순위:
 *   1) 인메모리 파일 캐시(같은 세션에서 이미 로드한 파일)
 *   2) 번들 레지스트리: public/examples-in-load.json (오프라인 OK)
 *   3) 웹 폴백: Supabase Storage 공개 버킷 'datasets' (큰/커스텀 데이터)
 * 어느 경로로 해석하든 결과는 파일 캐시에 적재되어 이후 조회는 즉시 반환된다.
 *
 * 설계 원칙: 가산적·하위호환. Pyodide 실행 경로/내보낸 Python/verify 픽스처 불변.
 * 입력층(LoadData.parameters.fileContent)만 채운다 — 이미 본문이 있으면 no-op.
 *
 * DFA 적응: 데이터 로더 타입이 LoadData·LoadClaimData 2종이고, 현재 .mla 샘플은
 * 본문(fileContent)을 내장하므로 바인딩은 대개 no-op(무해)이다. 본문 없이 파일명만
 * 가진 샘플이 올 때를 대비한 가산적 안전장치다.
 */

import { cacheFileContent, getCachedFileContent } from "./fileContentCache";

/** 웹 폴백/업로드에 사용할 Supabase Storage 버킷 이름. 없으면 조용히 건너뛴다. */
...
export async function uploadDatasetToWeb(
  ...
  try {
    ...
    cacheFileContent(name, content); // 즉시 사용 가능하도록 세션 캐시 적재
...
async function loadBundledRegistry(): Promise<Map<string, string>> {
  ...
  bundledRegistryPromise = (async () => {
    ...
    try {
      ...
      /* 번들 레지스트리가 없는 앱(예: DFA)에서는 조용히 빈 맵 */
    }
    bundledRegistry = map;
    return map;
  })();
...
async function fetchFromSupabaseStorage(filename: string): Promise<string | null> {
  try {
    // 동적 import — Supabase 클라이언트가 없는 앱에서도 빌드/실행 안전.
    const mod: any = await import("../lib/supabase").catch(() => null);
    const supabase = mod?.supabase;
    const isConfigured = mod?.isSupabaseConfigured;
    if (!supabase || (typeof isConfigured === "function" && !isConfigured())) {
...
/** 번들 레지스트리에서 사용 가능한 데이터셋 파일명 목록(원본 표기 보존은 불가, 소문자 키). */
export async function listBundledDatasets(): Promise<string[]> {
  const registry = await loadBundledRegistry();
  return Array.from(registry.keys());
}

/** 데이터 로더로 취급할 모듈 타입(DFA: LoadData·LoadClaimData, ML: LoadData). */
const LOADER_TYPES = new Set(["LoadData", "LoadClaimData", "XolLoading"]);
