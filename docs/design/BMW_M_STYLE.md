...
| 원칙 | 내용 |
|---|---|
| 캔버스 | 순수 검정(`#000`) 바닥 + 흰 타입. 라이트 모드는 시스템에 없다(토글로만 선택). |
...
### ④ `contexts/ThemeContext.tsx` — 기본은 항상 다크

`prefers-color-scheme` 분기를 지우고 `return 'dark'`. 라이트는 사용자가 토글로
명시 선택(localStorage)했을 때만. BMW M에는 라이트 서피스가 없다.
...
7. 라이트 토글 → 정상 복귀 + **라이트에서도 6번 재확인**(모드마다 안 보이는 자리가 다르다)
8. console error 0
