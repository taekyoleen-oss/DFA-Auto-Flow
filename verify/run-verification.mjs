// 전체 파이프라인 코드 검증 러너 (DFA-Auto-Flow).
//
// 무엇을 하나:
...
//
// 참고: DFA-Auto-Flow는 esbuild를 직접 의존하지 않지만, vite(>=6)가 esbuild를
// 전이 의존으로 포함한다. 아래 resolveEsbuild()가 vite 경유로 esbuild를 찾는다.
...
// ── esbuild 탐색 (직접 의존 없음 → vite 전이 의존 경유 폴백) ──────────────────
function resolveEsbuild() {
  // 1) 직접 해석 시도
  try {
    return require('esbuild');
  } catch {}
  // 2) vite 경유(전이 의존)로 해석
  try {
    const vitePkg = require.resolve('vite/package.json', { paths: [ROOT] });
    const esbuildPath = require.resolve('esbuild', { paths: [path.dirname(vitePkg)] });
    return require(esbuildPath);
  } catch {}
  // 3) 명시 경로 폴백
  try {
    return require(path.join(ROOT, 'node_modules', 'esbuild'));
  } catch {}
  throw new Error('esbuild를 찾을 수 없습니다 (vite 전이 의존 포함). `pnpm install` 후 다시 시도하세요.');
}
...
for (const fx of fixtures) {
  ...
  const ld = (fixture.modules || []).find((m) => m.type === 'LoadData' || m.type === 'LoadClaimData');
  const dsName = ld?.parameters?.source;
  const runDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dfaverify-'));
  let ok = false;
  let info = '';
  try {
    if (dsName) {
      const dp = findDataset(dsName);
      if (!dp) throw new Error(`데이터셋을 찾을 수 없음: ${dsName} (verify/datasets/public/Test 확인)`);
